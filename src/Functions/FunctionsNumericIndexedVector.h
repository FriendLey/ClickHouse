...
// template <template <template <typename, typename> class> class FuncImpl>
...
#undef DISPATCH
    ...
    ColumnPtr executeNumericIndexedVectorDataDispatch(const ColumnsWithTypeAndName & arguments, size_t input_rows_count) const
    {
        ...
        else
        {
            ...
            if (is_column_const[1])
                second_column_ptr = &typeid_cast<const ColumnVector<typename VectorImpl::ValueType> &>(typeid_cast<const ColumnConst &>(*value_column.get()).getDataColumn()).getData();
            else
            ...
            //                              ->getData();
            // else
            //     second_column_ptr = &typeid_cast<const ColumnVector<typename VectorImpl::ValueType> *>(arguments[1].column.get())->getData();
        ...
        size_t input_rows_count) const
    {
        ...
        for (size_t i = 0; i < input_rows_count; ++i)
        {
            ...
            auto lhs = reinterpret_cast<const AggregateFunctionGroupNumericIndexedVectorData<VectorImpl> *>(
                first_data_ptr);

            AggregateDataPtr second_data_ptr = is_second_column_const ? second_column_ptr->getData()[0] : second_column_ptr->getData()[i];
            auto rhs = reinterpret_cast<const AggregateFunctionGroupNumericIndexedVectorData<VectorImpl> *>(
                second_data_ptr);

            // check the name of operation (bitmapAnd) and check if it is the situation mentioned above
            col_to->insertDefault();
            auto res = reinterpret_cast<AggregateFunctionGroupNumericIndexedVectorData<VectorImpl> *>(
                col_to->getData()[i]);
            res->init = true;
...
#undef DISPATCH
    ...
    executeNumericIndexedVector(const ColumnsWithTypeAndName & arguments, const DataTypePtr & result_type, size_t input_rows_count) const
    {
        ...
        PaddedPODArray<typename VectorImpl::IndexType> & keys_pod = typeid_cast<ColumnVector<typename VectorImpl::IndexType> &>(*keys_data).getData();
        PaddedPODArray<typename VectorImpl::ValueType> & values_pod = typeid_cast<ColumnVector<typename VectorImpl::ValueType> &>(*values_data).getData();
