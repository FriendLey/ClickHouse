...
public:
        ...
        BSINumericIndexedVector & vector)
    {
        ...
        ValueType ratio = static_cast<ValueType>(1ULL << vector.fraction_bit_num);
        ...
        UInt32 & nonzero_cnt)
    {
        ...
        ValueType ratio = static_cast<ValueType>(1ULL << fraction_bit_num);
    ...
    static UInt16 valueToColumn(
        const BSINumericIndexedVector & vector,
        const std::shared_ptr<Roaring> & mask,
        const UInt32 & container_id,
        Float64 * output)
    {
        PaddedPODArray<UInt64> buffer(65536);
        ...
        {
            output[i] = static_cast<ValueType>(buffer[bit_buffer[i]]) / (1ULL << vector.fraction_bit_num);
        }
        return result_cnt;
    }

    static void pointwiseRawBinaryOperate(const BSINumericIndexedVector & lhs, const BSINumericIndexedVector & rhs,
                                          const UInt32 op_code, BSINumericIndexedVector & res)
    {
        ...
        for (const auto & container_id : container_ids)
        {
            ...
            LOG_DEBUG(log, "lhs_size: {}, rhs_size: {}", lhs_size, rhs_size);
            if (indexes_size != lhs_size || lhs_size != rhs_size)
                throw Exception(
                    ErrorCodes::INCORRECT_DATA, "{} keys_size({}), lhs_size({}), rhs_size({}) must equal in valueToColumn",
                    __func__, indexes_size, lhs_size, rhs_size);
            switch (op_code) {
    ...
    static void pointwiseRawBinaryOperate(const BSINumericIndexedVector & lhs, const ValueType & rhs,
                                          const UInt32 op_code, BSINumericIndexedVector & res)
    {
        auto and_all_indexes = lhs.allIndexes();
        ...
        for (const auto & container_id : container_ids)
        {
            ...
            if (indexes_size == 0)
                continue;
            UInt32 lhs_size = valueToColumn(lhs, and_all_indexes, container_id, lhs_values.data());
            LOG_DEBUG(log, "lhs_size: {}", lhs_size);
            if (indexes_size != lhs_size)
                throw Exception(ErrorCodes::INCORRECT_DATA, "{} keys_size({}), lhs_size({}) must equal in valueToColumn",
                                __func__, indexes_size, lhs_size);
            switch (op_code) {
                case multiply_op_code:
                    for (size_t i = 0; i < indexes_size; ++i)
                    {
...
public:
    ...
    // void pointwiseAdd(const NumericIndexedVector & rhs, NumericIndexedVector & res) const
    // {
    //     impl.pointwiseAdd(rhs.impl, res.impl);
    // }

    // void pointwiseAdd(const ValueType & rhs, NumericIndexedVector & res) const
    // {
    //     impl.pointwiseAdd(rhs, res.impl);
    // }
